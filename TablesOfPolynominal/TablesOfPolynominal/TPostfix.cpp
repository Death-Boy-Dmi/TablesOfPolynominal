...
bool TPostfix::CheckAmount() // Проверка соответствия кол-ва переменных кол-ву операций
{
	string str = " " + infix + " ";
	string arop = "-*/+";
...
bool TPostfix::CheckBrackets() // Проверка количества скобок
{
	string temp = infix;
...
bool TPostfix::CheckInfix() // Общая проверка
{
	CheckChars();
	CheckAmount();
	CheckBrackets();
...
void TPostfix::ArrVarible() // Выделение из строки переменных и добавление их в массив
{
	string inf = infix;
	size_t size = 0;
	CheckInfix();
...
TPolynominal TPostfix::CalculatePolynom(THashTable<TPolynominal> TableOfPolynom) // Ввод переменных, вычисление по постфиксной форме
{
